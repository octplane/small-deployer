...
impl Deployer {
    ...
    fn deploy(&self) {
        ...
        let status = r.run(&hk.cmd,
                           hk.parms,
                           "./logs".into(),
                           format!("deployer_{}", self.name),
                           Some(hk.pwd));
        ...
        match status {
            Ok(estatus) => {
                if estatus.success() {
                    let log_message = format!(":sunny: {} deployed successfully in {}s.",
                                              self.name,
                    ...
                    match estatus.code() {
                        ...
                        None => {
                            match estatus.signal() {
                                Some(signal_value) => {
                                    self.log(format!("Deploy was interrupted with signal {}.",
                                                     signal_value)
                                        .as_ref())
                                }
